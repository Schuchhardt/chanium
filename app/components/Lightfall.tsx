  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const resize = () => {
        ...
        (uniforms.iResolution.value as number[]) = [gl.drawingBufferWidth, gl.drawingBufferHeight, 1];
      };
      ...
      (container as unknown as Record<string, unknown>).__lightfallCleanup = () => {
    ...
    return () => {
      cancelled = true;
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      const cleanup = (container as unknown as Record<string, unknown>).__lightfallCleanup;
